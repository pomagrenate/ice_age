# iceage — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/JuliusBrussee/iceage/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the iceage hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/JuliusBrussee/iceage/main/hooks"

$HookFiles = @("package.json", "iceage-config.js", "iceage-activate.js", "iceage-mode-tracker.js", "iceage-statusline.sh", "iceage-statusline.ps1")
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("iceage")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasIceageHook "SessionStart") -and (& $hasIceageHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Iceage hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "iceage-activate.js"))) {
    Write-Host "Reinstalling iceage hooks (-Force)..."
} else {
    Write-Host "Installing iceage hooks..."
}
...
$env:ICEAGE_SETTINGS = $Settings -replace '\\', '/'
$env:ICEAGE_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.ICEAGE_SETTINGS;
const hooksDir = process.env.ICEAGE_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/iceage-statusline.ps1';
...
Write-Host "  - SessionStart hook: auto-loads iceage rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/iceage lite, /iceage ultra, /iceage-commit, etc.)"
Write-Host "  - Statusline badge: shows [ICEAGE] or [ICEAGE:ULTRA] etc."
