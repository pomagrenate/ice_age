...
$FlagFile = Join-Path $ClaudeDir ".iceage-active"
...
# Detect if iceage is installed as a plugin
$PluginInstalled = $false
$PluginsDir = Join-Path $ClaudeDir "plugins"
if (Test-Path $PluginsDir) {
    $found = Get-ChildItem -Path $PluginsDir -Recurse -Filter "plugin.json" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -match "iceage" }
    if ($found) { $PluginInstalled = $true }
}

if ($PluginInstalled) {
    Write-Host "Iceage appears to be installed as a Claude Code plugin." -ForegroundColor Yellow
    Write-Host "To uninstall the plugin, run:"
    Write-Host ""
    Write-Host "  claude plugin disable iceage" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host "Uninstalling iceage hooks..."
...
if (Test-Path $Settings) {
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "         Remove the iceage SessionStart and UserPromptSubmit"
...
Write-Host "  npx skills remove iceage       # Cursor, Windsurf, Cline, Copilot, etc."
Write-Host "  claude plugin disable iceage   # Claude Code plugin"
Write-Host "  gemini extensions uninstall iceage  # Gemini CLI"
