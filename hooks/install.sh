#!/bin/bash
# iceage — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/JuliusBrussee/iceage/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the iceage hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/JuliusBrussee/iceage/main/hooks"

HOOK_FILES=("package.json" "iceage-config.js" "iceage-activate.js" "iceage-mode-tracker.js" "iceage-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if ICEAGE_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.ICEAGE_SETTINGS, 'utf8'));
      const hasIceageHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('iceage'))
        );
      process.exit(
        hasIceageHook('SessionStart') &&
        hasIceageHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Iceage hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/iceage-activate.js" ]; then
  echo "Reinstalling iceage hooks (--force)..."
else
  echo "Installing iceage hooks..."
fi
...
chmod +x "$HOOKS_DIR/iceage-statusline.sh"
...
ICEAGE_SETTINGS="$SETTINGS" ICEAGE_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.ICEAGE_SETTINGS;
  const hooksDir = process.env.ICEAGE_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/iceage-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load iceage rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/iceage-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading iceage mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /iceage commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/iceage-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking iceage mode...'
      }]
    });
  }

  // Statusline — wire iceage badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — iceage badge NOT added.');
...
echo "  - SessionStart hook: auto-loads iceage rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/iceage lite, /iceage ultra, /iceage-commit, etc.)"
echo "  - Statusline badge: shows [ICEAGE] or [ICEAGE:ULTRA] etc."
