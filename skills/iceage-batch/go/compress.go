...
const (
	StatusOK            CompressStatus = iota
	StatusSkippedType                  // not natural language
	StatusSkippedBackup                // backup already exists
	StatusFailed                       // validation failed after retries
)
...
func callClaude(prompt string) (string, error) {
	...
	if apiKey != "" {
		...
		return stripLLMWrapper(strings.TrimSpace(msg.Content[0].Text)), nil
	}
...
// compressFile compresses a single file. quiet suppresses per-step output (used in batch mode).
func compressFile(path string, noBackup bool, quiet bool) (CompressStatus, error) {
	...
	if err != nil {
		return StatusFailed, fmt.Errorf("file not found: %s", absPath)
	}
	if info.Size() > maxFileSize {
		return StatusFailed, fmt.Errorf("file too large (max 500KB): %s", absPath)
	}
	if isSensitivePath(absPath) {
		return StatusFailed, fmt.Errorf("refusing to compress %s: filename looks sensitive", absPath)
	}

	if !shouldCompress(absPath) {
		return StatusSkippedType, nil
	}
	...
	if !noBackup {
		...
		if _, err := os.Stat(backupPath); err == nil {
			return StatusSkippedBackup, nil
		}
	}
